...
class Ui_ClinicData(object):
    def setupUi(self, ClinicData):
        ...
        try:
            ...
            self.texto_boasvindas.setText("Histórico de consultas")
            ...
            def funcao_tabela():
                manipulador.execute('SELECT * FROM consultas_db WHERE apagado = "s" ORDER BY id')
            ...
            def funcao_baixar():
                    try:
                            ...
                            if int(self.tableWidget.rowCount())!=0 and int(self.tableWidget.columnCount())!=0:
                                planilha = Workbook()
                                planilha.create_sheet(index = 0, title = "Histórico")
                                folha = planilha["Histórico"]
                                ...
                                if str(platform.system()) == str("Windows"): #Para salvar caso seja Windows
                                    ...
                                    if listdir(pasta_clinicdata) == [] or "historico.xlsx" not in listdir(pasta_clinicdata):
                                        nome = "historico.xlsx"
                                    else:
                                        numero = 1
                                        for k in range(0,len(listdir(pasta_clinicdata))):
                                            nome = str("historico" + "(" + str(numero) + ")" + ".xlsx")
                                    ...
                                    planilha.save("historico.xlsx")#Salva a planilha na pasta
            ...
            def funcao_pesquisar():
                ...
                if pesquisar != '':
                    ...
                    if string_codigo != '':
                        manipulador.execute(f'SELECT * FROM consultas_db WHERE apagado = "s" AND codigo LIKE "%{pesquisar}%"')
                    ...
                    if string_codigo == '':
                        manipulador.execute(f'SELECT * FROM consultas_db WHERE apagado = "s" AND paciente LIKE "%{pesquisar}%"')
                        lista = list(manipulador.fetchall())
                        self.tableWidget.setRowCount(len(lista))
                        cont = 0
                        while cont < len(lista):
